...
class LeagueDirectionsProvider extends \League\Container\ServiceProvider\AbstractServiceProvider implements \League\Container\ServiceProvider\BootableServiceProviderInterface
{
    ...
    {
        $this->getContainer()->add('Laasti\Directions\Strategies\HttpMessageStrategy',
            'Laasti\Directions\Strategies\HttpMessageStrategy');
        $this->getContainer()->add('Laasti\Directions\Strategies\PeelsStrategy',
            'Laasti\Directions\Strategies\PeelsStrategy')->withArgument('Laasti\Peels\Http\HttpRunner');

        if ($this->getContainer()->has('Laasti\Peels\Http\HttpRunner')) {
            $this->getContainer()->add('Laasti\Directions\Strategies\StrategyInterface',
                'Laasti\Directions\Strategies\PeelsStrategy')->withArgument('Laasti\Peels\Http\HttpRunner');
        } else {
            $this->getContainer()->add('Laasti\Directions\Strategies\StrategyInterface',
                'Laasti\Directions\Strategies\HttpMessageStrategy');
        }
        $this->getContainer()->add('Laasti\Directions\UrlBuilder')->withArguments([
            'Psr\Http\Message\ServerRequestInterface',
            'Laasti\Directions\RouteCollection'
        ]);

        $this->getContainer()->add('Laasti\Directions\RouterInterface',
            'Laasti\Directions\Router')->withArgument('Laasti\Directions\RouteCollection');
        $this->getContainer()->add('Laasti\Directions\RouteCollection',
            'Laasti\Directions\RouteCollection')->withArguments([
            'Laasti\Directions\Strategies\StrategyInterface'
        ]);
        ...
        foreach ($this->getConfig() as $name => $config) {
            ...
            $this->getContainer()->share('directions.collections.' . $name,
                'Laasti\Directions\RouteCollection')->withArguments([
                $config['strategy']
            ]);
            $this->getContainer()->share('directions.routers.' . $name,
                $config['router'])->withArguments(['directions.collections.' . $name]);
            $this->getContainer()->share('directions.builders.' . $name,
                $config['builder'])->withArguments([$config['request'], 'directions.collections.' . $name]);
        ...
        if (!is_null($alias)) {
            ...
            foreach ($names as $name) {
                if ($alias === 'directions.' . $name) {
                    return true;
                } elseif ($alias === 'directions.routers.' . $name) {
                    return true;
                } elseif ($alias === 'directions.builders.' . $name) {
                    return true;
                } elseif ($alias === 'directions.collections.' . $name) {
                    return true;
                }
            }
        }
