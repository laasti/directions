...
class GroupsTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testPrefixGroup()
    {
        ...
        $route = $group->add('GET', '/test', function () {
            return new TextResponse('TEST');
        });
        $this->assertEquals('/prefix/test', $route->getRoute());
        $this->assertInstanceOf('Zend\Diactoros\Response',
            $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/prefix/test')),
                new Response()));
    }
    ...
    public function testSuffixGroup()
    {
        ...
        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test.json')),
            new Response());
    ...
    {
        $router = $this->getRouter();
        $route = $router->add('GET', '/test', function () {
            return new TextResponse('TEST');
        });
        ...
        $route = $group->add('GET', '/test', function () {
            return new TextResponse('SECURE TEST');
        });

        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test')),
            new Response());
        $this->assertInstanceOf('Zend\Diactoros\Response', $response);
        $this->assertEquals('TEST', (string)$response->getBody());

        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test', 'p=2',
            'https')), new Response());
        $this->assertInstanceOf('Zend\Diactoros\Response', $response);
    ...
    public function testHostGroup()
    {
        ...
        $route = $group->add('GET', '/test', function () {
            return new TextResponse('HOST TEST');
        });
        ...
        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test', 'p=2', 'http',
            'example.com')), new Response());
        $this->assertInstanceOf('Zend\Diactoros\Response', $response);
    ...
    public function testConditionGroup()
    {
        ...
        $route = $group->add('GET', '/test', function () {
            return new TextResponse('CONDITION TEST');
        });

        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test', 'page=2'),
            ['page' => 2]), new Response());
        $this->assertInstanceOf('Zend\Diactoros\Response', $response);
        $this->assertEquals('TEST', (string)$response->getBody());

        $response = $router->dispatch(ServerRequestFactory::fromGlobals($this->fakeServerParams('/test', 'test=2'),
            ['test' => 2]), new Response());
        $this->assertInstanceOf('Zend\Diactoros\Response', $response);
